<x-shop::layouts.account>
    ...
    @php
        ...
        // Nombre del seller para el chat
        $shopName = 'Vendedor';
        if (isset($rmaData['order'])) {
            $marketplaceOrder = app('Webkul\Marketplace\Repositories\OrderRepository')
                ->where('order_id', $rmaData['order']->id)
                ->first();
    ...
    <div class="mx-4 flex-auto max-md:mx-6 max-sm:mx-4">
        ...
        <div class="mt-5 grid gap-5 max-1060:grid-cols-[1fr] max-md:mt-5">
            ...
            {{-- ═══ BLOQUE DE DISPUTA ═══════════════════════════════════════════
                 $dispute viene del controlador — sin @php, sin queries aquí.
                 Solo Blade puro: seguro fuera del x-template Vue.
            ════════════════════════════════════════════════════════════════ --}}
            @if ($dispute ?? null)
                <div class="relative mt-4 overflow-x-auto rounded-xl border">
                    <div class="px-8 py-5">
                        ...
                        <div class="mb-4 flex items-center justify-between">
                            ...
                            @elseif ($dispute->isApproved())
                                <span class="inline-flex items-center rounded-full bg-red-600 px-3 py-1 text-xs font-medium text-white">
                                    Disputa aprobada — RMA rechazado
                                </span>
                            @else
                                <span class="inline-flex items-center rounded-full bg-green-600 px-3 py-1 text-xs font-medium text-white">
                                    Disputa rechazada — tu solicitud continúa
                                </span>
                            @endif
                        </div>
                        ...
                        <div class="mb-1 text-xs font-medium uppercase tracking-wide text-gray-500">Observaciones del vendedor:</div>
                        <div class="mb-4 rounded-lg border bg-gray-50 p-4 text-sm leading-relaxed text-gray-700 dark:bg-gray-800 dark:text-gray-300">{{ $dispute->observations }}</div>
                                ...
                                @foreach ($dispute->images as $img)
                                    <a href="{{ Storage::url($img->path) }}" target="_blank" rel="noopener">
                                        <img src="{{ Storage::url($img->path) }}" alt="{{ $img->original_name }}"
                                             ...
                                             class="h-28 w-28 rounded-lg border object-cover transition hover:opacity-80">
                                    </a>
                                @endforeach
                            </div>
                        @endif
                        ...
                        @if (! $dispute->isPending())
                            <div class="border-t pt-4">
                                <div class="mb-1 text-xs font-medium uppercase tracking-wide text-gray-500">Resolución del administrador:</div>
                                @if ($dispute->admin_notes)
                                    <div class="rounded-lg border bg-gray-50 p-4 text-sm text-gray-700 dark:bg-gray-800 dark:text-gray-300">{{ $dispute->admin_notes }}</div>
                                @else
                                    <p class="text-sm italic text-gray-400">Sin notas adicionales.</p>
                                @endif
                            ...
                            <div class="flex justify-end">
                                ...
                                >
                                    <svg v-if="!isChatSend" aria-hidden="true" class="w-5 h-5 text-gray-200 animate-spin fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348...85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"/>
                                    </svg>
            ...
            app.component('option-wrapper', {
                ...
                methods: {
                    ...
                    handleFileSelect($event) {
                        ...
                        fileParagraph.textContent = fileNames.join(', ');
